...
class xcomfortAPI:
    ...
    async def set_shading_group_state(self, zone_id, state):
        result = await self.query('ShadingFunction/control', params=[zone_id, state])
        if not result['status'] == 'ok':
            return False
        else:
            return True

    async def get_shading_group_state(self, zone_id):
        return await self.query('ShadingFunction/getDashboard', params=[zone_id, ''])
